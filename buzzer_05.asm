...
;--------- time conditions
    ...
    mov a, minAlarm ;nope, need to borrow.
    anl a, #00FH
    cjne A, #0, borrowlsbMin ;can we subtract from lsb minutes?
    mov a, minAlarm ;nope, need to borrow
    anl a, #0F0H
    swap a
    cjne a, #0, borrowmsbMin ;can we borrow from msb minutes?
    mov a, hrsAlarm ;seconds decrementing
    anl a, #00FH
    cjne A, #0, borrowlsbHrs ;can we subtract from lsb hours?
    mov a, hrsAlarm ;nope, need to borrow
    anl a, #0F0H
    swap a
    cjne a, #0, borrowmsbHrs ;can we borrow from msb hours?
