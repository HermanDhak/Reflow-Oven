...
;--------- SECONDS
    mov a, secAlarm ;seconds decrementing
    anl a, #00FH
    cjne A, #0, regularSec ;can we subtract from lsb seconds?
    mov a, secAlarm ;nope, need to borrow
    anl a, #0F0H
    swap a
    cjne a, #0, borrowSec ;can we borrow from msb seconds?
    
borrowSec: ;borrow from msb seconds    
    swap a
    subb a, #1
    orl a, #10010000B
    swap a
    sjmp shiftBackSec
regularSec: ; yes, can subtract like regular
 	mov a, secAlarm
	subb a, #1  
shiftBackSec:
    mov secAlarm, a
    cjne A, #0FFH, ISR_End ;check to see if "-1" -> then we've overflowed, go back to 59
    mov secAlarm, #59H
    
;-------- MINUTES   ***********************************************************
 	mov a, minAlarm ;seconds decrementing
    anl a, #0FH
    cjne A, #0, regularMin ;if lsb minutes = 0, check if it can borrow from msb
   	mov a, minAlarm
   	cjne A, #0, decrementMinOK ; if msb ALSO  = 0, check if it can borrow from hrs
    mov A, hrsAlarm
    cjne A, #0, shiftBackMin ;if hrs ALSO = 0, leave it
    sjmp ISR_End ;for the case where min = 00, hrs = 00, we can't do any more decrements for either
DecrementMinOK: 
    mov a, minAlarm
    swap a
    subb a, #1
...
;--------- HOURS
	mov a, hrsAlarm ;hrs decrementing
    anl a, #0FH
    cjne A, #0, regularHrs
    mov a, hrsAlarm
    swap a
